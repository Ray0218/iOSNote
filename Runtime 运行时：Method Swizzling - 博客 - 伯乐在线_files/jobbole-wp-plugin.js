...
jQuery(document).ready(function ($) {
    ...
    $("#resource-submit-btn").bind("click", function () {
        ...
        var catId = $('#submit_form_cat_id').val();
...
function load_crawl_categories(site_id){
    if(site_id <= 0){
        alert('选择的频道不正确');
    }else{
        var select = document.getElementById("submit_form_category_id");
        ...
        $('#submit_form_category_id').selectpicker('refresh');
