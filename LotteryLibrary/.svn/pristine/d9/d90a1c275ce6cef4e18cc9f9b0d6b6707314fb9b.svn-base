...
namespace lottlib {
    ...
    int LotteryLjy::NwRefresh(NetworkCaller caller) {
...
//        NetworkService::Get("/live/GetZqLiveMatchs?gameTypeId=110", 11, caller, CALLBACK(LotteryLjy, CallbackRefresh));
        ...
        NetworkService::Posts("/", json, 12, hash_id, LOTTLIB_CALLBACK(LotteryLjy, cbkRefresh));
...
//        AES_encrypt(in, out, &key);
    ...
    int LotteryLjy::cbkRefresh(int result, Message msg) {
        return 0;
    }
...
#pragma mark - 存储相关
    ...
    int LotteryLjy::SetPurchaseInfo(int multiple) {
        return basket_->SetPurchaseInfo(multiple, 1);
    }
    
    int LotteryLjy::SplitOrders() {
        return basket_->SplitOrders();
    }
    
    int LotteryLjy::GetOrderCount() {
        return basket_->GetOrderCount();
    }
    
    int LotteryLjy::GetOrderInfo(int order, int &multiple, int &note) {
        return basket_->GetOrderInfo(order, multiple, note);
    }
    
    int LotteryLjy::GetOrderTargetCount(int order) {
        return basket_->GetOrderTargetCount(order);
    }
    
    int LotteryLjy::GetOrderTarget(int order, int index, int &note, int num1[LJY_NUMBER_COUNT], int num2[LJY_NUMBER_COUNT], int num3[LJY_NUMBER_COUNT],
                                   int num4[LJY_NUMBER_COUNT], int num5[LJY_NUMBER_COUNT], int num6[LJY_NUMBER_COUNT], int num7[LJY_NUMBER_COUNT]) {
        int target[CELL_TARGET_NUM_MAX_LEN] = { 0 };
        int type = 0;
    ...
    int LotteryLjy::DeleteOrder(int order) {
